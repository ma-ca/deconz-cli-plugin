...
/*! Send ZCL Request ClusterCommand (Send Command)
   \return true if request was added to queue
 */
bool CliPlugin::sendZclCmdGenericRequest(deCONZ::Address dstAddress,
...
/*! Send ZCL Request ClusterCommand (Send Command)
   \return true if request was added to queue
 */
bool CliPlugin::sendZclCmdRequest(deCONZ::Address dstAddress,
...
/*! Send ZCL Request ClusterCommand (Send Command) Manufacturer Specific
   \return true if request was added to queue
 */
bool CliPlugin::sendZclCmdManuSpecRequest(deCONZ::Address dstAddress,
		uint8_t ep, uint16_t cluster, QByteArray payload, int manu) {
...
/*! Send ZCL Default Response
   \return true if request was added to queue
 */
bool CliPlugin::sendZclDefaultResponse(const deCONZ::ApsDataIndication &ind, const deCONZ::ZclFrame &zclFrame, quint8 status) {
    char strtmp[TMP_STR];
    int rc = -1;
    deCONZ::ApsDataRequest apsReq;
...
/*! Send ZDP Match Request
   \return true if request was added to queue
 */
bool CliPlugin::sendZdpMatchCmdRequest(int profile, int cluster) {
...
/*! Send ZDP Request (for example Zdp_match_req)
   \return true if request was added to queue
 */
bool CliPlugin::sendZdpCmdRequest(deCONZ::Address dstAddress, uint16_t zdpcommand, QByteArray asdu) {
    deCONZ::ApsDataRequest apsReq;
    char strtmp[TMP_STR];
    int rc = -1;
 ...
 *
 */
void CliPlugin::get_dst_start_end(int finddst /* 0 or 1 */, time_t *time_dst_start, time_t *time_dst_end, int *time_dst_shift) {
	struct tm tm1, tm2;
 ...
 *
 */
void CliPlugin::get_dst(time_t *time_dst_start, time_t *time_dst_end, int *time_dst_shift) {
 ...
 *
 */
long CliPlugin::get_epoch_since_2000(time_t tinput) {
